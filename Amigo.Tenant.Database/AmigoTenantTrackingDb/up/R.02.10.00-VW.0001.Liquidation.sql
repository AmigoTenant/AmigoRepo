...
AS      
SELECT  I.InvoiceNo     ,ISNULL(I.TotalAmount, 0) AS TotalInvoice    , 

ISNULL(IncomeConcept.TotalIncome, 0) AS TotalIncome  ,I.InvoiceId     ,P.Code as PeriodCode        ,
H.Name as HouseName        ,T.TenantId    ,P.PeriodId    ,T.FullName as TenantFullName        ,
I.InvoiceDate      ,I.CustomerName    ,I.PaymentOperationNo    ,
I.BankName    ,I.PaymentOperationDate      ,I.Comment      ,C.ContractCode      ,I.TotalRent        ,
I.TotalDeposit        ,I.TotalLateFee        ,I.TotalService
,I.TotalFine        ,I.TotalOnAcount        ,I.TotalAmount        ,ID.PaymentPeriodId              ,
I.ContractId        ,GT.Value as PaymentTypeValue        ,GT.Code as PaymentTypeCode          ,
CPTO.Description as PaymentDescription        ,
CPTO.Code as ConceptCode,
ID.TotalAmount as PaymentAmount      ,P.DueDate as periodDueDate        ,
GT.Sequence as PaymentTypeSequence        ,
CHARINDEX(CPTO.Code, (SELECT AppSettingValue FROM AppSetting WHERE Code= 'CPTTOFAVTN' AND RowStatus = 1)) as IsTenantFavorable    ,US.UserName  
  ,T.Email    ,FR.FileRepositoryId  
  ,CES.Code AS ContractStatusCode
 FROM Invoice I        
 ...
 CROSS APPLY     (   
 SELECT SUM(ISNULL(TotalAmount, 0)) as TotalInvoice   
 FROM INVOICE I2   WHERE I2.TenantId = T.TenantId and      
 I2.PeriodId = P.PeriodId and      I2.RowStatus = 1    ) as TotalInvoice     
 
 CROSS APPLY     (   SELECT SUM(ISNULL(PP2.PaymentAmount,0)) as TotalIncome   
 FROM PaymentPeriod PP2     INNER JOIN Concept C2 ON C2.ConceptId = PP2.ConceptId   
 WHERE PP2.TenantId = T.TenantId and      PP2.PeriodId = P.PeriodId and      PP2.RowStatus = 1 and      
